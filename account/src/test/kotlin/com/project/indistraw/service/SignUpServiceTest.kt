...
class SignUpServiceTest: BehaviorSpec({
    ...
    val signUpService = SignUpService(commandAccountPort, queryAccountPort, passwordEncodePort, authenticationValidator, publisher, accountPublishPort)
    ...
    Given("signUpDto가 주어질때") {
        ...
        every { accountPublishPort.create(account) } returns Unit
        ...
        When("회원가입 요청을 하면") {
            ...
            Then("계정 생성 event가 각 서비스에 publish 되어야 한다..") {
                verify { accountPublishPort.create(account) }
            }
